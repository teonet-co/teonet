...
bool authSecretCheck(char *secret, uint8_t *data, uint32_t current_time) {
    ...
    int len = snprintf(0, 0, "%s%s%s", id, secret, timestamp);
    char *localAuth = malloc(len);
    snprintf(localAuth, len, "%s%s%s", id, secret, timestamp);
