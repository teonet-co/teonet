...
static ksnet_arp_data* ksnLNullSendFromL0(ksnLNullClass* kl, teoLNullCPacket* packet, char* cname,
                                          size_t cname_length);
...
static void cmd_l0_read_cb(struct ev_loop* loop, struct ev_io* w, int revents) {
  ...
  // Success read. Process package and resend it to teonet
    ...
    if(kld != NULL) {
      ...
      // Process read buffer
      while(kld->read_buffer_ptr - ptr >=
            (len = sizeof(teoLNullCPacket) + packet->peer_name_length + packet->data_length)) {

        // Check checksum
        if(packet->header_checksum ==
               get_byte_checksum(packet,
          ...
          // data = client_name
          //
          if(packet->cmd == 0 && packet->peer_name_length == 1 && !packet->peer_name[0] &&
             packet->data_length) {
...
static ksnet_arp_data* ksnLNullSendFromL0(ksnLNullClass* kl, teoLNullCPacket* packet, char* cname,
  ...
  memcpy(spacket->from + spacket->from_length, packet->peer_name + packet->peer_name_length,
         spacket->data_length);
...
#ifdef DEBUG_KSNET
    ksn_printf(kev, MODULE, DEBUG_VV, "send command from L0 \"%s\" client to \"%s\" peer ...\n",
               spacket->from, packet->peer_name);
#endif

    arp_data = ksnCoreSendCmdto(kev->kc, packet->peer_name, CMD_L0, spacket, out_data_len);
  }
  // Send to this host
  else {
#ifdef DEBUG_KSNET
 ...
 */
int ksnLNullSendToL0(void* ke, char* addr, int port, char* cname, size_t cname_length, uint8_t cmd,
...
#endif
  int rv = ksnCoreSendto(((ksnetEvMgrClass*)ke)->kc, addr, port, CMD_L0_TO, out_data, out_data_len);
 ...
 * @return Return 0 if success; -1 if data length is too lage (more than 32319)
 */
int ksnLNullSendEchoToL0(void* ke, char* addr, int port, char* cname, size_t cname_length,
                         void* data, size_t data_len) {

  size_t data_e_length;
  void* data_e =
      ksnCommandEchoBuffer(((ksnetEvMgrClass*)ke)->kc->kco, data, data_len, &data_e_length);

  int retval =
      ksnLNullSendToL0(ke, addr, port, cname, cname_length, CMD_ECHO, data_e, data_e_length);
  free(data_e);
  return retval;
}

int ksnLNullSendEchoToL0A(void* ke, char* addr, int port, char* cname, size_t cname_length,
                          void* data, size_t data_len) {
...
void _send_subscribe_events(ksnetEvMgrClass* ke, const char* name, size_t name_length) {
  ...
  teoSScrSend(ke->kc->kco->ksscr, EV_K_L0_CONNECTED, (void*)name, name_length, 0);
    ...
    if(kld != NULL) {
      ...
      for(;;) {
        ...
        ksnTRUDPsendto(((ksnetEvMgrClass*)(kl->ke))->kc->ku, 0, 0, 0, packet->cmd,
                       ((ksnetEvMgrClass*)(kl->ke))->kc->ku->fd, pkg, len, 0,
...
void ksnLNullClientDisconnect(ksnLNullClass* kl, int fd, int remove_f) {
  ...
  if(kld != NULL) {
    ...
    if(kld->name != NULL && remove_f != 2)
      teoSScrSend(kev->kc->kco->ksscr, EV_K_L0_DISCONNECTED, kld->name, kld->name_length, 0);

    // Free name
    if(kld->name != NULL) {

      if(remove_f) pblMapRemoveFree(kl->map_n, kld->name, kld->name_length, &valueLength);
 ...
 *
 */
static void cmd_l0_accept_cb(struct ev_loop* loop, struct ev_ksnet_io* w, int revents, int fd) {
...
static int ksnLNullStart(ksnLNullClass* kl) {
  ...
  if(kev->ksn_cfg.l0_allow_f) {
    ...
    int port_created;
    if((fd = ksnTcpServerCreate(kev->kt, kev->ksn_cfg.l0_tcp_port, cmd_l0_accept_cb, kl,
                                &port_created)) > 0) {

      ksn_printf(kev, MODULE, MESSAGE, "l0 server fd %d started at port %d\n", fd, port_created);
...
int cmd_l0_cb(ksnetEvMgrClass* ke, ksnCorePacketData* rd) {
  ...
  if(data->cmd == CMD_ECHO || data->cmd == CMD_ECHO_ANSWER || data->cmd == CMD_PEERS ||
     data->cmd == CMD_L0_CLIENTS || data->cmd == CMD_RESET || data->cmd == CMD_SUBSCRIBE ||
     data->cmd == CMD_SUBSCRIBE_RND || CMD_UNSUBSCRIBE || data->cmd == CMD_L0_CLIENTS_N ||
     data->cmd == CMD_L0_STAT || data->cmd == CMD_HOST_INFO || data->cmd == CMD_GET_NUM_PEERS ||
     data->cmd == CMD_TRUDP_INFO || (data->cmd >= CMD_USER && data->cmd < CMD_192_RESERVED) ||
  ...
  // Wrong command
    ...
    ksn_printf(ke, MODULE, DEBUG_VV,
               ...
               ANSI_RED, data->cmd, data->from, data->data_length, ANSI_NONE);
#endif
  }
...
int ksnLNullClientIsConnected(ksnLNullClass* kl, char* client_name) {
  ...
  size_t valueLength;
  int* fd = pblMapGet(kl->map_n, client_name, strlen(client_name) + 1, &valueLength);
...
int cmd_l0_to_cb(ksnetEvMgrClass* ke, ksnCorePacketData* rd) {
  ...
  // If l0 server is no initialized
    ...
    ksn_printf(ke, MODULE, ERROR_M,
               "can't resend command %d to L0 client \"%s\" from peer \"%s\": "
               "%s"
               "the L0 module has not been initialized at this host%s\n",
...
#define find_tag(VAR, KEY, TAG)                                                                    \
  if(!(keys & KEY) && jsoneq(data, &t[i], TAG) == 0) {                                             \
                                                                                                   \
    VAR = strndup((char*)data + t[i + 1].start, t[i + 1].end - t[i + 1].start);                    \
    keys |= KEY;                                                                                   \
    i++;                                                                                           \
  }
...
int cmd_l0_check_cb(ksnCommandClass* kco, ksnCorePacketData* rd) {
  ...
  if(jp.userId && (fd_old = ksnLNullClientIsConnected(kl, jp.userId))) {
    ...
    if(kld != NULL) { ksnLNullClientDisconnect(kl, fd_old, 2); }
  }
  ...
  if(fd) {
    ...
    if(kld != NULL) {
      ...
      if(jp.userId != NULL) { // && jp.clientId != NULL) {
        ...
        // Set new name
        kld->name = ksnet_formatMessage("%s%s%s", jp.userId, jp.clientId ? ":" : "",
      ...
      size_t snd;
      char* ALLOW = ksnet_formatMessage("{ \"name\": \"%s\", \"networks\": %s }", kld->name,
...
teonet_client_data_ar* ksnLNullClientsList(ksnLNullClass* kl) {
  ...
  if(kl != NULL && kev->ksn_cfg.l0_allow_f && kl->fd) {
    ...
    data_ar = malloc(sizeof(teonet_client_data_ar) + length * sizeof(data_ar->client_data[0]));
    int i = 0;
    ...
    if(it != NULL) {
      while(pblIteratorHasPrevious(it) > 0) {
        ...
        if(data != NULL && data->name != NULL) {
          strncpy(data_ar->client_data[i].name, data->name, sizeof(data_ar->client_data[i].name));
          i++;
        }
      }
      pblIteratorFree(it);
...
inline size_t ksnLNullClientsListLength(teonet_client_data_ar* clients_data) {
  ...
  return clients_data != NULL ? sizeof(teonet_client_data_ar) +
                                    clients_data->length * sizeof(clients_data->client_data[0])
                              : 0;
}
...
static ssize_t packetCombineClient(trudpChannelData* tcd, char* data, size_t data_len,
  ...
  if(tcd->last_packet_ptr > 0) {
    ...
    if(tcd->read_buffer_ptr > 0) {
      memmove(tcd->read_buffer, (char*)tcd->read_buffer + tcd->last_packet_ptr,
              (int)tcd->read_buffer_ptr);
    }
  ...
  if(recieved > 0) {
    if(tcd->read_buffer_ptr == 0 &&
       ((teoLNullCPacket*)data)->header_checksum !=
           get_byte_checksum(data, sizeof(teoLNullCPacket) -
  ...
  if(tcd->read_buffer_ptr - tcd->last_packet_ptr > sizeof(teoLNullCPacket) &&
    ...
    uint8_t header_checksum =
        get_byte_checksum(packet, sizeof(teoLNullCPacket) - sizeof(packet->header_checksum));
    uint8_t checksum =
        get_byte_checksum(packet->peer_name, packet->peer_name_length + packet->data_length);
...
int ksnLNulltrudpCheckPaket(ksnLNullClass* kl, ksnCorePacketData* rd) {
  ...
  if(rd->data_len == cp->data_length + cp->peer_name_length + sizeof(teoLNullCPacket)) {
    ...
    uint8_t header_checksum =
        get_byte_checksum(cp, sizeof(teoLNullCPacket) - sizeof(cp->header_checksum));
    ...
    if(rc == -3) {
#ifdef DEBUG_KSNET
      ksn_printf(kev, MODULE, DEBUG_VV, "WRONG UDP PACKET %d\n", rc);
#endif
      return 1;
    } else if((rc != -3) && (rc <= 0)) {
#ifdef DEBUG_KSNET
      ksn_printf(kev, MODULE, DEBUG_VV, "Wait next part of large packet %d status = %d\n",
                 rd->data_len, rc);
#endif
      return 1;
    }
