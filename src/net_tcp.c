 ...
 *
 * TCP client/server based on ksnet even manager
 */
...
/******************************************************************************/
/* TCP Client/Server module functions                                         */
/*                                                                            */
/******************************************************************************/

/**
 * Initialize ksnTcpServer module
 *
...
int ksnTcpServerCreate(
    ...
    ksnet_printf(&((ksnetEvMgrClass*)kt->ke)->ksn_cfg, DEBUG,
            "TCP Server, create: Create TCP server at port %d\n", port);
    #endif
    ...
    if( (sd = ksnTcpServerStart(kt, &server_port)) > 0 ) {
        ...
        *port_created = server_port;
        ...
        ev_ksnet_io_start (kev->ksnet_event_mgr_loop, &w_accept->io,
...
int ksnTcpServerStart(ksnTcpClass *kt, int *port) {
    ...
    while(try_port--) {
        ...
        if ((sd = socket(AF_INET6, SOCK_STREAM, 0)) < 0) {
            ksnet_printf(&kev->ksn_cfg, ERROR_M, "TCP Server: socket error\n");
            return -1;
        }
        #else
        ...
        {
            ksnet_printf(ERROR, "TCPTunnel Server: socket error");
            return -1;
        }
        #endif
        ...
        if (setsockopt(sd, SOL_SOCKET, SO_REUSEADDR, &yes, sizeof(int)) == -1) {
            ...
            ksnet_printf(&kev->ksn_cfg, ERROR_M,
                         "TCP Server: can't set socket options\n");
            return -1;
        }
        ...
        if (bind(sd, (struct sockaddr*) &addr, sizeof(addr)) != 0) {
            ksnet_printf(&kev->ksn_cfg, ERROR_M,
                         "TCP Server: bind on port %d error\n", *port);
            close(sd);
            if(try_port) (*port)++;
        }
        else break;
    ...
    if (listen(sd, 2) < 0) {
        ksnet_printf(&kev->ksn_cfg, ERROR_M,
                     "TCP Server: listen on port %d error\n", *port);
        return -1;
    }
...
void ksnTcpServerAccept(struct ev_loop *loop, ev_io *w, int revents) {
    ...
    if(EV_ERROR & revents) {
        ...
        if(revents == EV_ERROR + EV_READ + EV_WRITE) {
            ...
            ksnet_printf(&ke->ksn_cfg, DEBUG,
                    "TCP Server: Server with FD %d was closed\n", w->fd);
            #endif
            ...
            #ifdef DEBUG_KSNET
            ksnet_printf(&ke->ksn_cfg, DEBUG,
                    "TCP Server, accept: got invalid event: 0x%04X\n", revents);
            #endif
        }
    ...
    if (client_sd < 0) {
        ksnet_printf(&ke->ksn_cfg, ERROR_M, "TCP Server, accept: accept error\n");
        return;
    }
...
//    total_clients ++;
    ...
    if(strstr(client_ip, IPV4_PREF) == client_ip) {
        ...
        #ifdef DEBUG_KSNET
        ksnet_printf(&ke->ksn_cfg, DEBUG,
                     "TCP Server: Client IP address is: %s\n", client_ip);
        #endif
    } else {
        client_family = AF_INET6;
        #ifdef DEBUG_KSNET
        ksnet_printf(&ke->ksn_cfg, DEBUG,
                     "TCP Server: Client IPv6 address is: %s\n", client_ip);
        #endif
    }
    #endif
    ...
    ksnet_printf(
        &ke->ksn_cfg,
        DEBUG,
        "TCP Server: Successfully connected with client (%d), from IP%s: %s.\n",
...
int ksnTcpClientCreate(ksnTcpClass *kt, int port, const char *server) {
    ...
    {
        ksnet_printf(&kev->ksn_cfg, ERROR_M, "Client-socket() error\n");
        return(-1);
    }
    else {
        #ifdef DEBUG_KSNET
        ksnet_printf(&kev->ksn_cfg, DEBUG, "Client-socket() OK\n");
        #endif
    }
    ...
    {
        ksnet_printf(&kev->ksn_cfg, ERROR_M, "Client-connect() error\n");
        close(sd);
        return(-1);
    }
    else
    #ifdef DEBUG_KSNET
    ksnet_printf(&kev->ksn_cfg, DEBUG, "Connection established...\n");
    #endif
